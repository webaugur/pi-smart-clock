...
use crate::layout::{CLOCK_CX, CLOCK_CY, SCREEN_H, SCREEN_W};
...
pub async fn update<P: Platform>(platform: &mut P) {
    ...
    platform
        .draw_circle(CLOCK_CX, CLOCK_CY, 190, 0xFFFFFF)
        .await;
    platform
        .draw_circle(
            CLOCK_CX,
    ...
    for i in 0..12 {
        ...
        let x1 = (CLOCK_CX as f32 + ang.sin() * 190.0) as i32;
        let y1 = (CLOCK_CY as f32 - ang.cos() * 190.0) as i32;
        let x2 = (CLOCK_CX as f32 + ang.sin() * 170.0) as i32;
        let y2 = (CLOCK_CY as f32 - ang.cos() * 170.0) as i32;
        platform.draw_line(x1, y1, x2, y2, amber, 2).await;
    }

    let rad = angle.to_radians();
    let hx = (CLOCK_CX as f32 + rad.sin() * 165.0) as i32;
    let hy = (CLOCK_CY as f32 - rad.cos() * 165.0) as i32;
    platform
